...
<Callout title="Simplified API" type="info">
  Helper methods eliminate the need for complex spreading patterns and make hooks much more readable.
</Callout>
...
if (process.env.NODE_ENV === 'development') {
  const devUrl = context.request.url.replace('api.example.com', 'dev-api.example.com');
