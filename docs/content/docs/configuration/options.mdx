...
```js
const api = createInstance({
  withCredentials: true,  // Sets credentials: 'include'
  mode: 'cors',
  headers: {
...
<Callout title="Fixed in Latest Version" type="info">
  The `withCredentials` option now properly sets `credentials: 'include'` as expected.
</Callout>
...
```js
const api = createInstance({
  cache: 'no-cache',        // or 'default', 'force-cache', 'only-if-cached'
  withCache: true,          // Enable Z-Fetch caching
