...
<Callout title="Smart Implementation" type="info">
  Z-Fetch automatically chooses the best transport method based on your needs - XMLHttpRequest for progress, fetch for everything else.
</Callout>
  ...
  const { data, error } = await GET('https://api.example.com/download/large-file.zip', {
    onDownloadProgress: (event) => {
      if (event.lengthComputable) {
        ...
        updateDownloadText(`${formatBytes(event.loaded)} / ${formatBytes(event.total)}`);
      } else {
...
const downloadLargeFile = async (url, filename) => {
  ...
  if (data) {
    ...
    console.error(`Download failed at ${formatBytes(downloadedBytes)}:`, error.message);
...
const api = createInstance({
  ...
  onUploadProgress: (event) => {
    console.log(`Global upload: ${Math.round(event.loaded/event.total*100)}%`);
  },
  onDownloadProgress: (event) => {
    console.log(`Global download: ${Math.round(event.loaded/event.total*100)}%`);
  }
});
