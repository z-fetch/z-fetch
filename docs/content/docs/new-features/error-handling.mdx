...
Define custom error messages for z-fetch internal errors (network failures, timeouts, etc.):
...
<Callout title="Backend Errors Not Mapped" type="warn">
  **Important:** Error mapping only applies to z-fetch internal errors (NETWORK_ERROR, TIMEOUT, CANCELED). Backend HTTP errors (400s, 500s) are returned as-is with the original `response.statusText` from your API, allowing your backend to control error messages.
</Callout>

### Pattern Matching

Error mapping supports flexible pattern matching for z-fetch internal errors:
...
// Backend errors are NOT mapped - they use original statusText
const result = await api.get("/users");
if (result.error) {
  // If error.status is a number (400-599), it's from your backend
  // message will be the original response.statusText
  console.log(result.error.message); // e.g., "Unauthorized", "Not Found"
