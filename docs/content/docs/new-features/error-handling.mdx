...
<Callout title="Backend Error Mapping is Optional" type="info">
  By default (`mapBackendErrors: false`), only z-fetch internal errors (NETWORK_ERROR, TIMEOUT, CANCELED) are mapped. Backend HTTP errors use the original `response.statusText`, allowing your backend to control error messages. Set `mapBackendErrors: true` to enable mapping for backend errors too.
</Callout>
...
if (result.error) {
  ...
  console.log(result.error.message); // Original or mapped based on mapBackendErrors
