...
Map z-fetch internal errors to user-friendly text:

```js
const api = createInstance({
  throwOnError: true, // Works with error mapping
  errorMapping: {
    // Map z-fetch internal errors only
...
// Backend HTTP errors (400s, 500s) are NOT mapped
// They use the original response.statusText from your API
```

<Callout title="Backend Errors Not Mapped" type="warn">
  Error mapping only applies to z-fetch internal errors (NETWORK_ERROR, TIMEOUT, CANCELED). Backend HTTP status errors are returned as-is.
</Callout>
