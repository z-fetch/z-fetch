...
<Callout title="Careful" type="error">
  These Options are only applicable when setting up a Z-Fetch instance. Not
  supported on per request basis.
</Callout>
...
- Failed or canceled GETs are not cached; subsequent calls re-hit the network.
- Background revalidation preserves cached data if the revalidation fails.
